            ...
            if message.author.id == 282859044593598464 and "Witamy" in message.content:
                ...
                if self.user_mention in message.content and any(
                    ...
                    await self.command_channel.send(
                        f"{message.author.display_name} otrzymuje **+10$** za przywitanie nowego ostatniego użytkownika! /top_cash /shop")

                    self.user_mention = ""

        # Eco Bumps
        if message.channel.id == 1255884970761916499:
            if message.author.id == 282859044593598464 and "Bumpujesz" in message.content:
                ...
                self.save_eco_points()
                await self.command_channel.send(
                    f"{bump_user_fetch.display_name} otrzymuje **+{bump_eco_points}** za bumpowanie Naszego serwera! /top_cash /shop")

        # Eco Gaming
        if message.channel.id in (1249465490439671899, 1349710032656142387, 1269979219451187242, 1269978371543007362, 1276875368497680475, 1361938010194837587, 1369658691158147152):
            for attachment in message.attachments:
                if attachment.filename.lower().endswith((".jpg", ".jpeg", ".png", ".mp4", ".mov", ".webm")):
                    ...
                    self.save_eco_points()
                    await self.command_channel.send(
                        f"{message.author.display_name} otrzymuje **+5$** za wysłanie obrazu/filmiku! /top_cash /shop")
